---
name: single-file-business-site
...
If the user provided neither URL nor description, ask once: "What business is this for? Paste a Google Maps URL, website link, or just tell me the name + city + type."
...
- **Never use em-dashes (—) or en-dashes (–) in website copy.** Use commas, colons, or restructure. This is a hard rule.
...
Save the file to:
```
~/Desktop/[business-name-kebab-case]-site/index.html
```
...
3. Auto-open via Bash:
   ```bash
   open ~/Desktop/[business-name-kebab]-site/index.html
   ```

Then tell the user in 2-3 lines:
- File path
...
This skill builds client/prospect demos that live on Desktop, not in the vault. No hub note backlink unless the user explicitly asks.
